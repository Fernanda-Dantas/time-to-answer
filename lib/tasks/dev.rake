...
task add_answers_and_questions: :environment do
  Subject.all.each do |subject|
    rand(5..10).times do |i|
      ...
      elect_true_asnwer(answers_array)
      Question.create!(params[:question])
    end
  end
end

desc "Reseta o contador de perguntas dos assuntos"
task reset_subject_counter: :environment do
  ...
  def create_question_params(subject = Subject.sample.all)
    { question: {
  ...
  def elect_true_asnwer(answers_array = [])
